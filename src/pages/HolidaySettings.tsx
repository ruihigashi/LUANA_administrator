...
const HolidaySettings: React.FC = () => {
  ...
  // 8) リセット（全休解除）：
  //    holidays から当日を削除し、
  //    time_slots を Reservation の情報をもとに復元
  const clearFullDayOff = async () => {
    ...
    try {
      ...
      const { data: reservationData, error: resError } = await supabase
        .from('reservations')
        .select<Reservation[]>('start_time, end_time')
        .eq('date', dateStr);
            ...
            {days.map((cell) => {
              ...
              return (
                <button
                  ...
                  className={`
                    relative flex flex-col items-center justify-center h-20 border rounded-md transition-colors duration-200
                    ${
                      isPastDay
